...
def update_graph(filename, contents, xaxis, yaxis):
    ...
    if not xaxis or not yaxis or not contents or not filename:
        return fig
    contents = contents[0]
    filename = filename[0]
    df = parse_contents(contents, filename)
...
def update_output(xaxis, yaxis, click_json, contents, filename):
    if not xaxis or not yaxis or not click_json or not contents or not filename:
        return [{}], []
    contents = contents[0]
    filename = filename[0]
    df = parse_contents(contents, filename)
    xval = click_json['points'][0]['x']
    yval = click_json['points'][0]['y']
    df = df[(df[xaxis] == xval) & (df[yaxis] == yval)]
